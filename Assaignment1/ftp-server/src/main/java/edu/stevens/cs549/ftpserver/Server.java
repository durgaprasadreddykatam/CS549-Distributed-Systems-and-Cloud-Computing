...
//    	clientSocket = s;
		...
		public void run() {
			try {
				...
				try {
					...
					/*
					 * zTODO: Complete this thread (remember to flush output!).
					 */
		...
		public void run() {
			try {
				...
				try {
					...
					byte[] buffer = new byte[1024];
					int bytesRead;// Read data from the socket input stream (from the client) and write it to the output stream (to the server's file).
	...
	public void get(String file) throws IOException, FileNotFoundException, RemoteException {
		if (!valid(file)) {
			...
			try {
				 ...
				 */
				OutputStream socketOutputStream = socket.getOutputStream();byte[] buffer = new byte[1024];
				int bytesRead;
				while ((bytesRead = in.read(buffer)) != -1) {
					socketOutputStream.write(buffer, 0, bytesRead);
				}socketOutputStream.flush();
	...
	public void put(String file) throws IOException, FileNotFoundException, RemoteException {
		if (!valid(file)) {
			...
			try {
				/*
				 * TODO: Connect to client socket to receive and write the file.
				 */
				InputStream socketInputStream = socket.getInputStream();
				byte[] buffer = new byte[1024];
				int bytesRead;
