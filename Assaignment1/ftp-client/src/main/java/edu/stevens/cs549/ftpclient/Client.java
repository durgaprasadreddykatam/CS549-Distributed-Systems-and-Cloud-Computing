...
public class Client {
	...
	public class Dispatch {
		...
		private int makeActive() throws IOException {
			...
			dataChan = new ServerSocket(0, BACKLOG_LENGTH, InetAddress.getByName("0.0.0.0"));
			mode = Mode.ACTIVE;
			/*
			...
			if (inputs.length == 2) {
				try {
					if (mode == Mode.PASSIVE) {
						...
						InputStream in = new BufferedInputStream(new FileInputStream(inputs[1]));
						PutThread putThread = new PutThread(dataChan, in);
						Thread thread = new Thread(putThread);
						thread.start();
						svr.put(inputs[1]);
					} else {
						msgln("GET: No mode set--use port or pasv command.");
